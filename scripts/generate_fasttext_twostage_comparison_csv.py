...
master_csv_path = fix_win_path("Comparison Tables - Final copy.csv")
out_csv_path = fix_win_path("Comparison Tables - FastText TwoStage Finetuned.csv")
out_csv_path_std = fix_win_path("Comparison Tables - FastText Finetuned.csv")
benchmark_results_dir = fix_win_path("data_pipeline/datasets/benchmark_results")
...
for i in range(len(df)):
    if str(df.iloc[i, 0]).strip() == "fastText LID-176":
        row_idx_zero = i
        break
...
if row_idx_zero is not None:
    for bname, col_map in benchmark_cols.items():
        pred_path = fix_win_path(os.path.join(benchmark_results_dir, f"fasttext_lid_176_{bname}.csv"))
...
if row_idx_ft is not None:
    for bname, col_map in benchmark_cols.items():
        pred_path = fix_win_path(os.path.join(benchmark_results_dir, f"fasttext_lid_176_twostage_all_langs_{bname}.csv"))
        if not os.path.exists(pred_path):
            pred_path = fix_win_path(os.path.join(benchmark_results_dir, f"fasttext_lid_176_finetuned_all_langs_{bname}.csv"))
...
with open("temp_fasttext_twostage.csv", "w", encoding="utf-8") as f:
    df.to_csv(f, index=False)
print("Saved temporary FastText Two-Stage CSV: temp_fasttext_twostage.csv")
