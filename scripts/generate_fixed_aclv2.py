...
def main():
    with open('ACLV2.tex', 'r', encoding='utf-8') as f:
        text = f.read()
    ...
    # Fix unescaped ampersand in itemize
    text = text.replace(
        r"\item \textbf{Short-Text & OCR Applications:}",
        r"\item \textbf{Short-Text \& OCR Applications:}"
    )

    # Convert all markdown **bold** to \textbf{bold}
    text = re.sub(r'\*\*([^*]+)\*\*', r'\\textbf{\1}', text)

    # 2. In preamble, add safety definitions for \citename if not present
    ...
    # 3. Create the 100% natbib-compatible thebibliography block
    natbib_bibitems = """\\begin{thebibliography}{27}
...
% If you prefer a single-file standalone LaTeX submission without an external custom.bib file,
% replace the \\bibliography{custom} line above with the self-contained natbib-compatible block below:
%
% \\input{thebibliography_block.tex}"""

    if '\\bibliography{custom}' in text:
        acl_v2_official = text
        acl_v2_standalone = text.replace(official_ref_block, natbib_bibitems)
        if acl_v2_standalone == text:
            acl_v2_standalone = text.replace('\\bibliography{custom}', natbib_bibitems)
    else:
        acl_v2_official = pattern.sub(lambda m: official_ref_block, text)
        acl_v2_standalone = pattern.sub(lambda m: natbib_bibitems, text)

    with open('ACLV2.tex', 'w', encoding='utf-8') as f:
        f.write(acl_v2_official)
    print("Updated ACLV2.tex with official \\bibliography{custom}!")

    with open('ACLV2_standalone.tex', 'w', encoding='utf-8') as f:
        f.write(acl_v2_standalone)
    print("Created ACLV2_standalone.tex with 100% natbib-compatible thebibliography!")

    with open('thebibliography_block.tex', 'w', encoding='utf-8') as f:
        f.write(natbib_bibitems)
    print("Created thebibliography_block.tex!")
