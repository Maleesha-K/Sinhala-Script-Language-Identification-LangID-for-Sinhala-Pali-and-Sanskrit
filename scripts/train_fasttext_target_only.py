"""Train ContinualLID (leaf-surgery FastText LID-176) STRICTLY on target data (train.csv).

Benchmark against the 3 hybrid datasets:
- FLORES+ (eval_flores_plus_11lang.csv)
- CommonLID (eval_commonlid_11lang.csv)
- WiLI-2018 (eval_wili_2018_11lang.csv)

Compare per-class F1, macro F1, and accuracy with the existing Comparison Tables.
"""
...
def map_pred_to_11(pred_lang, text):
    if pred_lang == "sa":
        ...
        if script == "Sinh":
            ...
            return "San-Sinh" # default
    return LANG_TO_11.get(pred_lang, pred_lang)

def load_target_data(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        if label in ["sinhala", "si", "sinh-sinh"]:
            model_label, eval_group = "si", "Sinhala-Sinh"
        elif label in ["pali", "pi", "pali-sinh"]:
            model_label, eval_group = "pi", "Pali-Sinh"
        elif label in ["sanskrit", "sa", "san-sinh"]:
            model_label, eval_group = "sa", "Sanskrit-Sinh"
        else:
            continue
        records.append({
...
def evaluate_on_benchmark(model, csv_path, bname):
    ...
    # raw_preds is list of [lang_code]
    pred_langs = [p[0] for p in raw_preds]

    # Map predictions to 11 classes
...
def main():
    ...
    train_csv = "data_pipeline/datasets/finetuning/train.csv"
    val_csv = "data/Nadil/val.csv"
    out_dir = "data_pipeline/models/continual/target_only_exp"
    os.makedirs(out_dir, exist_ok=True)

    print(f"Loading untrained ContinualLID from {init_dir}...")
    model = ContinualLID.from_pretrained(init_dir)

    print(f"Loading training data from {train_csv} (Target Only)...")
    train_records = load_target_data(train_csv)
    print(f"Loaded {len(train_records)} training records:")
    train_counts = {}
    for r in train_records:
        train_counts[r['model_label']] = train_counts.get(r['model_label'], 0) + 1
    print("  Counts:", train_counts)

    print(f"Loading validation data from {val_csv}...")
    val_records = load_target_data(val_csv)
    ...
    # Hyperparameters as established in RESULTS_LID176.md
    epochs = 5
    lr = 0.5
    batch_size = 64
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0, weight_decay=0)

    print(f"\nStarting training: {epochs} epochs, lr={lr}, batch_size={batch_size} on TARGET ONLY data...")
    best_val_acc = 0.0
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch} complete: Train Loss = {total_loss/total_samples:.4f}, Val Target Accuracy = {val_acc:.4f}")

        if val_acc > best_val_acc:
            best_val_acc = val_acc
            model.save_pretrained(os.path.join(out_dir, "best"))
            print(f"  * Saved new best model to {out_dir}/best")
    ...
    results_file = os.path.join(out_dir, "target_only_benchmark_results.json")
