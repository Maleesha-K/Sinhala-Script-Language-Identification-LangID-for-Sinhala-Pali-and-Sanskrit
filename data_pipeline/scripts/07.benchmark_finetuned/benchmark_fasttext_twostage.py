"""
================================================================================
Two-Stage Hierarchical Evaluation Pipeline for FastText LID-176
================================================================================
Purpose:
  Stock FastText LID-176 correctly routes global background languages (English, German,
  French, Tamil, etc.), but classifies ALL text written in Sinhala script as 'si'.
  
  Conversely, a fine-tuned FastText LID-176 model trained only on Sinhala, Pali, and Sanskrit
  ...
  Solution: Two-Stage Hierarchical Pipeline Architecture
    - Stage 1 (Stock FastText LID-176 Router): Receives input text. If Stage 1 detects any
      global non-Sinhala-script language (e.g. English, Tamil, Devanagari Sanskrit),
      it immediately returns that global prediction.
    - Stage 2 (Fine-Tuned FastText LID-176 Specialist): If Stage 1 detects 'si'
  ...
  This script benchmarks the Two-Stage FastText LID-176 pipeline against hybrid FLORES+,
...
# ------------------------------------------------------------------------------
# Locate Stock FastText LID-176 base model (Stage 1 Router)
stock_model_path = os.path.join(proj_root, "data_pipeline", "models", "benchmark", "fastText", "lid.176.bin")
if not os.path.exists(stock_model_path):
    stock_model_path = os.path.join(proj_root, "models", "benchmark", "fastText", "lid.176.bin")

# Path to local Fine-Tuned FastText LID-176 model weights (Stage 2 Specialist)
finetuned_model_path = os.path.join(proj_root, "data_pipeline", "models", "finetuned", "fastText_LID_176", "fasttext_lid_176_finetuned.bin")

print(f"Loading Stock FastText LID-176 (Stage 1 Router) from {stock_model_path}...")
stock_model = fasttext.load_model(fix_win_path(stock_model_path))

print(f"Loading Fine-Tuned FastText LID-176 (Stage 2 Specialist) from {finetuned_model_path}...")
finetuned_model = fasttext.load_model(fix_win_path(finetuned_model_path))

# ------------------------------------------------------------------------------
# 4. Taxonomy & Label Mapping Dictionaries
# ------------------------------------------------------------------------------
# Maps FastText LID-176 (ISO 639-1) 2-letter codes to common benchmark names
STAGE1_TO_COMMON = {
    "en": "english",
    "de": "german",
    "fr": "french",
...
def clean_text_fasttext(text):
    """
    Cleans raw text for FastText inference by trimming whitespace and replacing newlines.
    """
...
def predict_two_stage(text):
    """
    Executes Two-Stage Hierarchical Classification for FastText LID-176:
    
    Stage 1: Stock FastText LID-176 (Router)
      - Predicts top language label (k=1).
      - If predicted label IS NOT 'si' (ISO 639-1 code for Sinhala script):
        - Return mapped global label (e.g. 'english', 'tamil', 'german', etc.).
        
    Stage 2: Fine-Tuned FastText LID-176 (Specialist)
      - If Stage 1 predicted 'si' (Sinhala script text), pass text to Stage 2.
      - Stage 2 predicts fine-grained target label ('sinhala', 'pali', or 'sanskrit').
    """
    # Stage 1: Stock FastText LID-176 Prediction
    p1, _ = stock_model.predict(text, k=1)
    ...
    # Stage 2: Fine-Tuned FastText LID-176 Specialist for Sinhala script
    p2, _ = finetuned_model.predict(text, k=1)
...
for fname in benchmark_files:
    ...
    print(f"\nEvaluating Two-Stage FastText LID-176 Pipeline on {len(df_all)} samples from {dataset_name}...")
    
    # Clean text and execute 2-stage inference
    texts = df_all["text"].apply(clean_text_fasttext).tolist()
    preds = [predict_two_stage(t) for t in texts]
    ...
    print("=" * 70)
    print(f"TWO-STAGE HIERARCHICAL BENCHMARK RESULTS (FastText LID-176 - Evaluated on {dataset_name})")
    print("=" * 70)
    ...
    out_csv = fix_win_path(os.path.join(results_dir, f"fasttext_lid_176_twostage_all_langs_{dataset_name}.csv"))
    results.to_csv(out_csv, index=False)
    print(f"Saved two-stage benchmark predictions to {out_csv}")

print("\n" + "=" * 55)
print("TWO-STAGE FASTTEXT LID-176 BENCHMARKING COMPLETE")
print("=" * 55)
